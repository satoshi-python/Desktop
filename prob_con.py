...
def main():
    ...
    for j1 in range(6):
        ...
        # plt.figure(figsize=(20, 20))
    fig.savefig("MED26_hy.pdf")
    for j1 in range(6):
        f = open("{0}-HY.txt".format(str(txt_path[j1])[7:]), "w")
        print(str(txt_path[j1])[7:])
        A = [a + b + c + d + e for (a,b,c,d,e) in zip(TXT[j1][0],TXT[j1][1],TXT[j1][2],TXT[j1][3],TXT[j1][4])]
        print(len(A))
        for i in a:
            AA = str(i) + "\n"
            f.write(AA)
