    ...
    for j6 in range(kizami):
        for j7 in range(kizami):
            if clt[j6][j7] > 0.0:
                ...
                clt5[j6][j7] += -1
    YY_x(clt5, kizami, y1, x1, savepath, y_min, y_haba,
         x_min, x_haba, more, mini)


def YY_x(clt, kizami, y1, x1, savepath, y_min, y_haba,
         x_min, x_haba, more, mini):
    prob = []
    for i in range(kizami):
        for j in range(kizami):
    ...
    for j in range(len(x2)):
        ...
        for j1 in range(len(x2[j])):
            ...
            for j2 in range(len(x1)):
                a = int(kizami - (int((y1[j2] - y_min)/y_haba) + 1))
                if a == int(x2[j][j1]):
                    b = int((int((x1[j2] - x_min)/x_haba) - 1))
                    if b == int(y2[j][j1]):
                        if j2 not in num:
                            num.append(j2)
...
def kaki_x(savepath, num, num1, num2, num3):
    f = open("{0}/{1}(PC1:{3}_PC2:{2}).txt".format(savepath,
                                                   str(num1),
                                                   str(num2),
                                                   str(num3)), "w")
...
def P_tyuusyutu(text, kizami, Prob, savepath, li="satoshi"):
    ...
    clt = [[0.0 for i in range(kizami)] for j in range(kizami)]
    for j6 in range(len(x1)):
        a = kizami - (int((y1[j6] - y_min)/y_haba) + 1)
        b = (int((x1[j6] - x_min)/x_haba) - 1)
        clt[a][b] += float(prob_kai[j6])
        ...
        for i in range(len(x1)):
            if int(kizami - (int((y1[i] - y_min)/y_haba) + 1)) == int(x[h2]):
                if int((int((x1[i] - x_min)/x_haba) - 1)) == int(y[h2]):
                    print("kizami:", kizami)
                    print(int(kizami - (int((y1[i] - y_min)/y_haba) + 1)),
                          int((int((x1[i] - x_min)/x_haba) - 1)))
                    num.append(i)
        kaki(savepath, num)
        print("end_YY")
...
def free_energy(path, kizami, savepath, probpath, FONT=int(14), ligand="satoshi"):
    ...
    for i in range(kizami):
        ...
        y.append(y3)
    for j6 in range(len(x1)):
        a = (int((y1[j6] - y_min)/y_haba) + 1)
        b = (int((x1[j6] - x_min)/x_haba)) - 1
        clt[kizami - a][b] += float(prob_kai[j6])
    print("end")
    ...
    ax = sns.heatmap(df, cmap="jet", vmin=0, vmax0=10, center=cen,
                     mask=df_mask, cbar_kws={'label': 'PMF'})
    ...
    """
    # plt.xticks([0,kizami/4,kizami/2,kizami*3/4,kizami],
                 [x_min,x_min+(x_haba * kizami/4),0,
                 x_max-(x_haba * kizami/4),x_min])
    # plt.yticks([0,kizami/4,kizami/2,kizami*3/4,kizami],
                 [y_max,y_max-(y_haba * kizami/4),0,
                 y_min+(y_haba * kizami/4),y_min])
    """
    plt.xticks([0, kizami/4, kizami/2, kizami*3/4, kizami],
               [round(x_min, 2), round(x_min+(x_haba * kizami/4), 2),
               round((x_max+x_min)/2), round(x_max-(x_haba * kizami/4), 2),
               round(x_max, 2)])
    plt.yticks([0, kizami/4, kizami/2, kizami*3/4, kizami],
               [round(y_max, 2), round(y_max-(y_haba * kizami/4), 2),
               round((y_max+y_min)/2), round(y_min+(y_haba * kizami/4), 2),
               round(y_min, 2)])
